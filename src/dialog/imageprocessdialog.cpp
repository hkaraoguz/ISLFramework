...
void ImageProcessDialog::on_butApplyAll_clicked()
{
    ...
    if(ui->cBoxOmni->isChecked())
    {
        bool okAngle = false;
        bool okNoImages = false;

        double angleOffset =  ui->lEditAngleOffsetOmni->text().toDouble(&okAngle);
        int noImages = ui->lEditNoImagesOmni->text().toInt(&okNoImages);
            ...
            for(int i = 0; i < filters.size(); i++)
            {
                ...
                for(int k = 0; k < noBasePoints ; k++)
                {
                    ...
                    for(int j = 0; j < noImages; j++)
                    {
                        ...
                        vector<bubblePoint> imgBubble = bubbleProcess::convertGrayImage2Bub(sonuc,focalLengthPixels,255, j*angleOffset);
                        ...
                        resred = bubbleProcess::reduceBubble(filterBubbleWhole);
                        ...
                        if(ui->cBoxBubble->isChecked())
                        {
                            ...
                            DatabaseManager::insertBubble(filterNumber,k+1,resred);
